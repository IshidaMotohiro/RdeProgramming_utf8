...
# コンソールから実行する場合 # weight <- read.csv("weight.csv")
...
t.test (weight$after, weight$before, paired = TRUE)

## # なお、以下のように「前後」を示すカテゴリ変数が独立した列になっている場合
(weight2 <- stack(weight[, -1]))
##    values    ind
## 1   48.73 before
## 2   49.87 before
## 3   53.39 before
...
# 以下のように検定を実行することもできます
t.test(values ~ ind, paired = TRUE, data = weight2)
